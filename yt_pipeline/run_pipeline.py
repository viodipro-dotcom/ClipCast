...
- AI metadata: requires OPENAI_API_KEY (configure in app Settings → Integrations).
- Optional cloud transcription: set OPENAI_CLOUD_TRANSCRIPT=1 and OPENAI_API_KEY to use OpenAI whisper-1 instead of local Whisper.
"""
...
_env_cpu_threads = (os.getenv("WHISPER_CPU_THREADS", "") or "").strip()
...
def resolve_whisper_cpu_threads() -> int:
    if _env_cpu_threads:
        try:
            value = int(_env_cpu_threads)
            if value > 0:
                return value
        except Exception:
            pass
...
def build_whisper_kwargs(device: str, compute_type: str) -> Dict[str, Any]:
    kwargs = {"device": device, "compute_type": compute_type}
    if device == "cpu":
        kwargs["cpu_threads"] = resolve_whisper_cpu_threads()
    return kwargs
    ...
    try:
        ...
        if "cuda" in error_str or "cudnn" in error_str or "dll" in error_str:
            ...
            try:
                ...
                model = WhisperModel(
                    WHISPER_MODEL_NAME,
                    **build_whisper_kwargs(device, compute_type),
                )
...
def _openai_api_v1_base() -> str:
    """Base URL ending in /v1 for chat and (with same host) compatible APIs."""
    raw = (os.getenv("OPENAI_BASE_URL") or "https://api.openai.com/v1").strip().rstrip("/")
    if raw.endswith("/v1"):
        return raw
    return f"{raw}/v1"


def _call_openai_direct_chat(
    ...
    try:
        ...
        msg = msg or raw or str(e)
        raise ValueError(f"OpenAI API error: {msg}")

    choices = data.get("choices")
    if not isinstance(choices, list) or not choices:
        raise ValueError("OpenAI did not return choices")
...
def transcribe_openai_cloud(mp3_path: Path) -> str:
    """Transcribe audio using OpenAI hosted whisper-1 (requires openai package)."""
    try:
        from openai import OpenAI
    except ImportError as e:
        raise RuntimeError(
            "The 'openai' package is required for cloud transcription. Install: pip install openai"
        ) from e

    api_key = os.getenv("OPENAI_API_KEY", "").strip()
    if not api_key:
        raise ValueError("OPENAI_API_KEY is not set for cloud transcription")

    base_v1 = _openai_api_v1_base()
    client = OpenAI(api_key=api_key, base_url=base_v1)
...
def call_openai_proxy(
    ...
    if not os.getenv("OPENAI_API_KEY", "").strip():
        raise ValueError(
            "Missing OPENAI_API_KEY. Add your API key in the app under Settings → Integrations."
        )
    return _call_openai_direct_chat(messages, model, response_format, temperature)
...
def main() -> int:
    ...
    use_cloud_tr = os.getenv("OPENAI_CLOUD_TRANSCRIPT", "").strip().lower() in ("1", "true", "yes")
    has_openai_key = bool(os.getenv("OPENAI_API_KEY", "").strip())
    if use_cloud_tr and not has_openai_key:
        print("ERROR: OPENAI_CLOUD_TRANSCRIPT requires OPENAI_API_KEY (configure in app Settings → Integrations).")
        return 2

    if use_cloud_tr and has_openai_key:
        model = None  # type: ignore[assignment]
        print("☁️  Transcription: OpenAI cloud (whisper-1). Local Whisper model will not be loaded.")
    else:
    ...
    for v in videos:
        ...
        try:
            ...
            if not transcript_path.exists() or transcript_path.stat().st_size < 10:
                if use_cloud_tr and has_openai_key:
                    text = transcribe_openai_cloud(mp3_path)
