...
export default function App() {
  ...
  const [authEntitlement, setAuthEntitlement] =
    React.useState<AuthEntitlementSnapshot>(OPEN_ACCESS);
  const authEntitlementRef = React.useRef<AuthEntitlementSnapshot>(OPEN_ACCESS);
  React.useEffect(() => {
  ...
  const handleBillingError = React.useCallback((err: unknown, _kind?: 'metadata' | 'upload'): boolean => {
    if (handleNetworkError(err)) {
      return true;
    }
    setSnack('Request failed. Try again.');
    return false;
  ...
  const guardUploadAndScheduleAccess = React.useCallback((): boolean => true, []);
...
React.useEffect(() => {
  ...
  const loadAuthAndEntitlement = React.useCallback(async () => {
    const snapshot = await loadAuthAndEntitlementState();
  ...
  const handleOpenExternal = React.useCallback(
    async (url: string) => {
      ...
      if (!opener) {
        setSnack(t('openLinkUnavailable'));
        return;
      }
    ...
    <ThemeProvider theme={theme}>
      ...
      {networkStatus === 'offline' && (
        ...
        >
          ...
          >
            ...
            Internet connection is required for metadata generation and uploads.
          </Alert>
        </Box>
      )}
      <Box
      ...
      >
        ...
        <SettingsDialog
          ...
          onSnack={(msg) => setSnack(String(msg))}
          onYouTubeStateMayChange={() => void refreshYtConnected()}
        />
