...
async function assistNextDueJob() {
  ...
  if (!alreadyOpened) {
    if (process.env.E2E_TEST === '1') {
      log(`[auto-upload][E2E_TEST] assist overlay simulated for ${platform} ${job?.filePath}`);
    } else {
...
async function manualAssist(platform, job, silentMode = false) {
  // E2E_TEST: avoid opening browser/Explorer or notifications in automated runs.
  // We just log and let processDueJobs persist "assistAt" as the de-dupe marker.
  if (process.env.E2E_TEST === '1') {
    log(`[auto-upload][E2E_TEST] manualAssist(${platform}) simulated for ${job?.filePath}`);
    return 0;
  }
