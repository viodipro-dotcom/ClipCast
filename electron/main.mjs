...
async function initIpcHandlers() {
  ...
  function loadUiSettings() {
    try {
      if (!fs.existsSync(uiSettingsFile)) {
        return { uiLanguage: 'en', runAtStartup: false, openAiCloudTranscript: false };
      }
      ...
      return {
        ...
        openAiCloudTranscript: Boolean(parsed?.openAiCloudTranscript),
      };
    } catch (e) {
  ...
  ipcMain.handle('settings:set', (_e, payload) => {
    ...
    const openAiCloudTranscript =
      typeof patch.openAiCloudTranscript === 'boolean' ? patch.openAiCloudTranscript : current.openAiCloudTranscript;
    const next = {
      ...current,
      ...patch,
      runAtStartup,
      openAiCloudTranscript: Boolean(openAiCloudTranscript),
    };
    saveUiSettings(next);
    return next;
  });
  ...
  ipcMain.handle('secrets:getOpenAIApiKeyStatus', async () => {
    const status = await getOpenAIApiKeyStatus();
    return { ok: true, ...status };
  });
  ipcMain.handle('secrets:setOpenAIApiKey', async (_e, key) => {
  ...
  ipcMain.handle('secrets:clearOpenAIApiKey', async () => {
    await clearOpenAIApiKey();
    return { ok: true };
  });
...
function readOpenAiCloudTranscriptFlag() {
  try {
    const p = path.join(app.getPath('userData'), 'ui_settings.json');
    if (!fs.existsSync(p)) return false;
    const parsed = JSON.parse(fs.readFileSync(p, 'utf8'));
    return Boolean(parsed?.openAiCloudTranscript);
  } catch {
    return false;
  }
}
...
ipcMain.handle('pipeline:runPipeline', async (_event, payload) => {
  ...
  if (pref === 'force_cpu') {
    ...
    try {
      computeInfo = await getComputeBackend({ forceRefresh: true, log, source: 'pipeline', userInitiated: true });
    } catch (e) {
      computeInfo = null;
      fallbackReason = String(e);
    }
    let availableGpu = Boolean(computeInfo && computeInfo.availableGpu);
    let nvidiaPresent = Boolean(computeInfo && computeInfo.details && computeInfo.details.nvidia_present);
    let smokeReason = computeInfo?.details?.cuda_smoke?.reason || computeInfo?.error || null;
    if (!availableGpu && nvidiaPresent && smokeReason === 'no_cuda_device') {
      log('INFO', 'compute', 'Retrying CUDA probe after no_cuda_device', {});
      await new Promise((resolve) => setTimeout(resolve, 1200));
      try {
        computeInfo = await getComputeBackend({ forceRefresh: true, log, source: 'pipeline_retry', userInitiated: true });
        availableGpu = Boolean(computeInfo && computeInfo.availableGpu);
        nvidiaPresent = Boolean(computeInfo && computeInfo.details && computeInfo.details.nvidia_present);
        smokeReason = computeInfo?.details?.cuda_smoke?.reason || computeInfo?.error || null;
      } catch (e) {
        computeInfo = null;
        fallbackReason = String(e);
        availableGpu = false;
    ...
    if (availableGpu) {
      ...
      effectiveDevice = 'cpu';
      if (nvidiaPresent) {
        fallbackReason = smokeReason || fallbackReason || 'cuda_unavailable';
      } else {
  ...
  const openaiKeyForEnv =
    (process.env.OPENAI_API_KEY && String(process.env.OPENAI_API_KEY).trim()) || (await getOpenAIApiKey()) || '';
  const useOpenAiCloudTranscript = readOpenAiCloudTranscriptFlag();
  if (useOpenAiCloudTranscript && !openaiKeyForEnv) {
    log('ERROR', 'pipeline', 'Cloud transcription requires an OpenAI API key', {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      dialog
        .showMessageBox(mainWindow, {
          ...
          message: 'Cloud transcription is enabled but no OpenAI API key is configured.',
          detail: 'Add your API key in Settings → Integrations, or turn off cloud transcription.',
        })
        .catch(() => {});
    }
    return { runId, code: 2, error: 'OpenAI API key required for cloud transcription' };
  }
  ...
  const pipelineEnv = buildPythonEnv(python, {
    ...
    ...(openaiKeyForEnv ? { OPENAI_API_KEY: openaiKeyForEnv } : {}),
    ...(useOpenAiCloudTranscript && openaiKeyForEnv ? { OPENAI_CLOUD_TRANSCRIPT: '1' } : {}),
